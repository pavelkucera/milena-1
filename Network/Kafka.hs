...
makeRequest :: (Serializable req, Deserializable resp, Kafka m) => Handle -> ReqResp req resp -> m resp
...
metadata' h request = makeRequest h $ MetadataRR (MetadataRequestV0 request)
