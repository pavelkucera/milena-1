...
fetchRequest :: Kafka m => Offset -> Partition -> TopicName -> m (FetchRequest FetchRequestV2 FetchResponseV2)
fetchRequest o p topic = do
  wt <- use stateWaitTime
  ws <- use stateWaitSize
  bs <- use stateBufferSize
  return . FetchRequestV2 $ FetchReqV2 (ordinaryConsumerId, wt, ws, [(topic, [(p, o, bs)])])

-- | Execute a fetch request and get the raw fetch response.
fetch' :: Kafka m => Handle -> FetchRequest FetchRequestV2 FetchResponseV2 -> m FetchResponseV2
fetch' h request = makeRequest h $ FetchRR request

fetch :: Kafka m => Offset -> Partition -> TopicName -> m FetchResponseV2
fetch o p topic = do
...
fetchMessages :: FetchResponseV2 -> [TopicAndMessage]
fetchMessages fr = (fr ^.. fetchResponseFieldsV2 . _2 . folded) >>= tam
